...
import { blockchainNetworks } from '../data/providers';
...
export default function CryptoPanel({ language, showNetworks = false }: CryptoPanelProps) {
  ...
  const { cryptoPrices, isLoading, lastUpdated, refresh, isUsingDemoData } = useMarketData(60000);
    ...
    <div className="space-y-6">
      ...
      {isUsingDemoData && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
          <p className="text-sm text-amber-700 dark:text-amber-400">
            ⚠️ {language === 'es' 
              ? 'Usando datos de demostración. Configura VITE_COINGECKO_API_KEY en .env para datos en tiempo real.'
              : 'Using demo data. Configure VITE_COINGECKO_API_KEY in .env for real-time data.'}
          </p>
        </div>
      )}
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          📡 {isUsingDemoData 
            ? (language === 'es' ? 'Fuente: Datos de demostración' : 'Source: Demo data')
            : (language === 'es' ? 'Fuente: CoinGecko API' : 'Source: CoinGecko API')}
        </p>
